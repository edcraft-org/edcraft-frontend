...
interface TemplatePreviewProps {
  preview: GenerateTemplatePreviewResponse;
}

export function TemplatePreview({ preview }: TemplatePreviewProps) {
  const { question_text, question_type, template_config, preview_question } = preview;
        ...
        <CardContent className="space-y-2">
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              ...
              <span className="ml-2 font-mono">{template_config.entry_function}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Output Type:</span>
              <span className="ml-2">{template_config.question_spec.output_type}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Question Type:</span>
              <span className="ml-2">{template_config.question_spec.question_type}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Distractors:</span>
              <span className="ml-2">{template_config.generation_options.num_distractors}</span>
            </div>
          </div>
        </CardContent>
      </Card>
