...
export function QuestionCard({
  ...
  const [internalShowAnswer, setInternalShowAnswer] = useState(defaultShowAnswer);

  // Use controlled state if provided, otherwise use internal state
  const showAnswer = controlledShowAnswer ?? internalShowAnswer;
  const setShowAnswer = controlledShowAnswer === undefined ? setInternalShowAnswer : undefined;
  ...
  const hasOptions = isMCQOrMRQ && "options" in additional_data && Array.isArray(additional_data.options);
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Show/Hide Answer Button - Only show if not controlled */}
        {setShowAnswer && (
          <div>
            <Button
              type="button"
        ...
        {showAnswer && (
          <div className="p-3 bg-green-50 dark:bg-green-950 border border-green-500 dark:border-green-700 rounded-md">
            <h4 className="text-sm font-semibold text-green-800 dark:text-green-300 mb-2">Answer:</h4>
